...
var QuizDB = (function() {
    ...
    var TYPE_TAG_MAP = {
        '单选': 'single', '多选': 'multiple', '判断': 'tf',
        '填空': 'fill', '简答': 'short', '分析': 'analysis'
    };
    ...
    function parseBankQuestions(md) {
        ...
        for (var i = 0; i < blocks.length; i++) {
            ...
            var type = header.explicitType || detectQuestionType(parsed.text, parsed.options, normalized.answer);
    ...
    function parseQuestionHeading(line) {
        ...
        if (numeric) {
            var title = numeric[3] ? numeric[3].trim() : '';
            var explicitType = null;
            var typeMatch = title.match(/^\[([^\]]+)\]\s*/);
            if (typeMatch) {
                explicitType = TYPE_TAG_MAP[typeMatch[1]] || null;
                title = title.slice(typeMatch[0].length);
            }
            return {
