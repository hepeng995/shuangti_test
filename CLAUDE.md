...
bank-database.js — 内置数据库题库数据（110 单选 + 45 判断 + 4 SQL 填空，调用 registerBank() 注册）
bank-web.js     — 内置动态Web技术题库数据（90 单选 + 20 判断 + 4 简答 + 1 填空，调用 registerBank() 注册）
...
`db.js` → `router.js` → `quiz-core.js` → `quiz-select.js` → `admin.js` → `bank-java.js` → `bank-database.js` → `bank-web.js` → 内联启动脚本

### 数据流

1. **启动**: 各模块 IIFE 执行 → `bank-java.js`、`bank-database.js`、`bank-web.js` 分别调用 `QuizDB.registerBank()` 注册内置题库 → 内联脚本初始化路由 → 根据 hash 渲染对应页面
...
缓存名为 `quiz-platform-v9`（在 `sw.js` 中定义）。更新静态资源后必须递增缓存名，否则客户端无法获取新版本。`CACHE_FILES` 数组列出了所有预缓存的文件（当前包含 `bank-java.js`、`bank-database.js` 和 `bank-web.js`），新增 JS 文件需同步更新此数组。

### 添加新题库

1. 创建新的 `bank-xxx.js` 文件，调用 `QuizDB.registerBank({ id, name, mdContent })` 注册
2. 在 `index.html` 中最后一个 bank JS 之后添加 `<script src="bank-xxx.js"></script>`
